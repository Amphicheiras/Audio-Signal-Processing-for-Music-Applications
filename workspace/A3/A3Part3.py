...
def testRealEven(x):
    ...
    # compute the DFT of the zero phase windowed version
    X = fft(dftbuffer)

    # check if the signal is real and even
    # isRealEven = np.allclose(X[:N//2+1], np.conj(X[-1:-N//2-1:-1]), atol=1e-6) and np.isreal(x)
    isReal = bool(np.all(abs(np.imag(X) < 1e-6)))
    isEven = bool(np.all(np.real(X[2*M:M:-1])) == np.all(np.real(X[0:M])))
    isRealEven = isReal and isEven
