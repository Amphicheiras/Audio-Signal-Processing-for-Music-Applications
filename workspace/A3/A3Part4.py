...
def suppressFreqDFTmodel(x, fs, N):
    ...
    # compute DFT of sound fragment
    magX, phasX = dftAnal(x, w, N)
    magX_filt = magX.copy()

    # apply filtering
    k = int(np.ceil(70.0 * N / fs))
    magX_filt[:k+1] = -120.0

    # reconstruct each signal
    y = dftSynth(magX, phasX, w.size) * sum(w)
    yfilt = dftSynth(magX_filt, phasX, w.size) * sum(w)
