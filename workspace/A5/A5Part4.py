...
def selectFlatPhasePeak(pX, p, phaseDevThres):
    ...
    selectFlag = False
    # index = p-2, p-1, p, p+1, p+2
    samples = pX[p-2:p+3]
    # mean value of phases
    mean = sum(samples) / len(samples)
    # X of sigma formula
    square_diff = [(pX_current - mean) ** 2 for pX_current in samples]
    # computation of standard deviation
    sigma = np.sqrt(sum(square_diff) / len(samples))

    # compare with threshold
    if sigma < phaseDevThres:
        selectFlag = True
    ...
    while pin < pend:  # while input sound pointer is within sound
        ...
        for pindex, p in enumerate(ploc):
            if p > 2 and p < (len(pX) - 2):  # Peaks at either end of the spectrum are not processed
                if selectFlatPhasePeak(pX, p,
                                       phaseDevThres):  # Select the peak if the phase spectrum around the peak is flat
        ...
        if len(plocSel) != 2:  # Ignoring frames that don't return two selected peaks
            ...
            iploc, ipmag, ipphase = UF.peakInterp(mX, pX,
                                                  plocSel)  # Only selected peaks to refine peak values by interpolation
