...
def estimateInharmonicity(inputFile='../../sounds/piano.wav', t1=0.1, t2=0.5, window='hamming',
    ...
    hfreq, _, _ = HM.harmonicModelAnal(x, fs, w, N, H, t, nH, minf0, maxf0, f0et, 0.01, 0.0)

    # 2. Extract the time segment in which you need to compute the inharmonicity.
    nFrames = len(hfreq[0:])
    l1 = int(np.ceil((nFrames / (len(x) / fs)) * t1))
    l2 = int(np.ceil((nFrames / (len(x) / fs)) * t2))
    f0 = hfreq[:, 0]
    ...
    for l in range(nFrames):
        ...
        for r in range(1,nH):
            if hfreq[int(l), r] > 0:
                I[int(l)] = I[int(l)] + abs(hfreq[int(l), r] - (r + 1) * f0[int(l)]) / (r + 1)
            else:
                zeros += 1
        # print(nH - zeros)
        I[int(l)] = I[int(l)] / (nH - zeros)
    # compute mean inharmonicity
    meanInharm = 1 / (l2 - l1 + 1) * sum(I[l1:l2+1])
    return meanInharm
