...
def segmentStableNotesRegions(inputFile='../../sounds/sax-phrase-short.wav', stdThsld=10, minNoteDur=0.1,
    ...
    nFrames = len(f0)
    melody_index = np.array([])
    # 1. convert f0 values from Hz to Cents (as described in pdf document)
    f0_cents = 1200 * np.log2(f0 + 1e-6 / 55.0)

    # 2. create an array containing standard deviation of last winStable samples
    sigma = np.zeros(nFrames)
    for index in range(nFrames):
        sigma[index] = np.std(f0_cents[int(index):int(index+winStable)])

    # 3. apply threshold on standard deviation values to find indexes of the stable points in melody
    melody_index = np.array([index for index in range(nFrames) if sigma[index] < stdThsld])

    # 4. create segments of continuous stable points such that consecutive stable points belong to same segment
    segments = np.zeros(shape=(1, 2))
    start_seg = melody_index[0]
    end_seg = melody_index[0]
    for item in range(len(melody_index[1:])):
        if melody_index[int(item)] == end_seg + 1:
            end_seg = melody_index[int(item)]
        else:
            segments = np.vstack((segments, np.array([start_seg, end_seg])))
            start_seg = melody_index[item]
            end_seg = melody_index[item]

    duration_threshold = minNoteDur * (nFrames / (len(x) / fs))
    segments = segments[(segments[:, 1] - segments[:, 0]) >= duration_threshold]
...
def plotSpectogramF0Segments(x, fs, w, N, H, f0, segments):
    ...
    for ii in range(segments.shape[0]):
        plt.plot(timeStamps[int(segments[ii, 0]):int(segments[ii, 1])], f0[int(segments[ii, 0]):int(segments[ii, 1])], color='#A9E2F3',
                 linewidth=1.5)
...
# print(segmentStableNotesRegions())