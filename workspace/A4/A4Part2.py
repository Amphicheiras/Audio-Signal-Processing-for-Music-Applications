...
def computeSNR(inputFile, window, M, N, H):
    ...
    # append nulls to x to match length of y
    x = np.concatenate((x, np.zeros(len(y) - len(x))))
    # calculate input signal energy for entire length
    E_signal_entire = np.sum(np.abs(x) ** 2)
    # calculate noise energy for entire length
    E_noise_entire = np.sum(pow((x-y), 2))

    # calculate signal energies
    E_signal_windowed = np.sum(np.abs(x[M:len(x) - M]) ** 2)
    # calculate signal energies for certain window
    E_noise_windowed = np.sum(np.abs(x[M:len(x) - M]-y[M:len(y) - M]) ** 2)

    # calculate SNRs
    SNR1 = 10 * np.log10(E_signal_entire / E_noise_entire)
    SNR2 = 10 * np.log10(E_signal_windowed / E_noise_windowed)
