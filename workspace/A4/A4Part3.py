...
def computeEngEnv(inputFile, window, M, N, H):
    ...
    Output:
            ...
            containing energy envelopes of the signal in decibles (dB) scale
    ...
    # STFT analysis
    mX_dB, _ = stft.stftAnal(x, w, N, H)
    # return to linear scale
    mX_lin = 10.0 ** (mX_dB / 20.0)

    # find freq bins
    freqs = np.linspace(0, fs / 2, (N // 2) + 1, endpoint=True)
    k0 = [int(N * f / fs) for f in freqs if 0 < f < 3000]
    k1 = [int(N * f / fs) for f in freqs if 3000 < f < 10000]
    # separate lowpass and highpass samples
    E_lowpass = 10 * np.log10(np.sum(mX_lin[:,k0] ** 2, axis=1) + 1e-16)
    E_highpass = 10 * np.log10(np.sum(mX_lin[:,k1] ** 2, axis=1) + 1e-16)

    # print(f"Lowpass:{k0 + 1} - {k1 - 1} Highpass: {k1} - {k2 - 1}")

    # calculate lowpass and highpass energy
    numFrames = len(mX_lin[:, 0])
...
# computeEngEnv('../../sounds/piano.wav', 'blackman', 513, 1024, 128)