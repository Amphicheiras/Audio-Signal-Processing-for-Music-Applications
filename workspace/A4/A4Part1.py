...
def extractMainLobe(window, M):
    ...
    minima_indices, _ = find_peaks(-mX1)

    left_minima = np.max(minima_indices[minima_indices < N/2])
    right_minima = np.min(minima_indices[minima_indices > N/2])

    main_lobe = mX1[left_minima:right_minima+1]
